...
import { LIMIT_PER_CATEGORY, BLOG_CATEGORY_ID } from '@/_constants/blog';
...
export default async function BlogPage(): Promise<JSX.Element> {
  ...
  const categoryList: MicroCMSListResponse<Category>[] = await Promise.all(
    BLOG_CATEGORY_ID.map((categoryId: string) => {
  ...
  const categoryNames: CategoryName[] = categoryList.map(
    (data: MicroCMSListResponse<Category>, index: number) => {
      ...
      if (!firstCategory) {
        return {
          categoryId: BLOG_CATEGORY_ID[index],
          categoryEN: "",
          categoryJP: "",
        };
      }
    ...
    <>
      <div className={`${styles.flexWrapper}`}>
        ...
        <main>
          ...
          <BlogContent blogArea={blogs} categoryList={categoryNames} pageName={pageName} />
        </main>
